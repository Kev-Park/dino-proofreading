...
def annotate_terminate(input_path):
    # --- Setup ---
    ...
    if input_path == None:
        input_path = filedialog.askdirectory(title="Select folder with images")
        if not input_path:
            print("No folder selected. Exiting.")
            exit(1)

    image_files = [f for f in sorted(os.listdir(input_path)) if f.lower().endswith(SUPPORTED_FORMATS)]
    ...
    for img_file in image_files:
        full_path = os.path.join(input_path, img_file)
